...
_AimSelectWidget100554_decorators = [customElement("aim-select-widget-100554")];
class AimSelectWidget100554 extends (_a = CollabLitElement, _listcontainer_dec = [query(".select-grid")], _a) {
  ...
  get value() {
    ...
    const values = Array.from(all).map((inp) => inp.checked ? inp["file"] : void 0).filter((item) => !!item);
    return values;
  }
  getFilesNames() {
    const { project } = mls.actual[5];
    if (project === void 0) throw new Error("Invalid project selected");
